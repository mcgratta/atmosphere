...
\end{eqnarray}
where $\mathbf{J_{d}}_\alpha=- \rho D_\alpha \boldsymbol{\nabla} Y_\alpha$ are the diffusive fluxes for component $\alpha$. Fick's Law for binary diffusion respect to a background species is assumed, $D_\alpha$ is the diffusivity of $\alpha$ respect to the background species.  To enforce realizability, $\sum_\alpha \mathbf{J_{d}}_\alpha = 0$, we lump errors in diffusive transport into the most abundant species locally \cite{McDermott:2015}. The volumetric combustion source term for species $\alpha$ is $\dot{m}_\alpha'''(\mathbf{x},t)$  and $\dot{m}_{b,\alpha}'''$ is the contribution to species $\alpha$ from subgrid particle gasification. In the momentum equation the head is $H=|\mathbf{u}|^2/2 + \tilde{p}/\rho$, $\tilde{p}$ is the perturbation pressure, and $\rho_0(z)$ is an equlibrium densit.  The gravity vector is $\mathbf{g}=(0,0,g_z)$, $\mathbf{f}_{b}$ is the particle force term, and $\boldsymbol{\tau}$ is a tensor accounting for molecular and subgrid turbulent stresses parameterized by an effective eddy viscosity model. This last model is employed by default in large-eddy simulation (LES) of outdoor fires. Details of the model equations and their derivation are found in \cite{mcgratta_2013tr}.
...
\begin{figure}[ht]
\includegraphics[width=\textwidth]{figures/Case_F19_0358.png}
...
Measured ROS                   & m/s           & 1.0           & 1.0           \\ \hline
Calculated ROS, Particle Method & m/s           & 1.0           & 1.0           \\ \hline
Calculated ROS, Boundary Fuel Method & m/s           & 1.0           & 1.0           \\ \hline
Calculated ROS, Level Set Method & m/s           & 1.0           & 1.0           \\ \hline
\end{tabular}
\end{center}
\end{table}
...
\subsection{Cogoleto Fire}  
\label{sec:cogo}

At approximately 23:00 (local time) on March 25, 2019, a wildfire started in the seaside town of Cogoleto, near Geno, Italy. Strong winds, gusting up to 100~km/h, spread the fire from its ignition point on a ridge above the town towards the sea. 
